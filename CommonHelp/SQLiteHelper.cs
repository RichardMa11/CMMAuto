    ...
    public class SQLiteHelper
    {
        ...
        static bool CreateTable(string db)
        {
            try
            {
                ...
                SQLiteConnectionStringBuilder conStr = new SQLiteConnectionStringBuilder { DataSource = db };

                using (var conn = new SQLiteConnection(conStr.ToString()))
                {
                    conn.Open();
                    using (var cmd = new SQLiteCommand(conn))
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        //量测数据表
                        //cmd.CommandText = "DROP TABLE IF EXISTS MeaSureData";
                        //cmd.ExecuteNonQuery();
